    ...
    return (
      <Layout>
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
          ...
          <p className="font-heading font-bold text-xl uppercase">Admin Access Required</p>
        </div>
      </Layout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center gap-2 font-mono text-xs text-primary uppercase tracking-widest mb-3">
            <Layers className="w-3.5 h-3.5" /> Admin Panel
          </div>
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-heading font-black uppercase tracking-tight">DASHBOARD</h1>
              <p className="text-muted-foreground font-mono mt-1">
                {products.length} products · Logged in as <span className="text-primary">{user.username}</span>
              </p>
            </div>
            {view === "list" && (
              <button
        ...
        <AnimatePresence mode="wait">
          ...
          {view === "edit" && editProduct && (
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-heading font-bold text-xl uppercase flex items-center gap-2">
                  <Pencil className="w-5 h-5 text-primary" /> Edit Product
                </h2>
        ...
        {view === "list" && (
          <div>
            ...
            {loading ? (
              <div className="space-y-2">
                {Array(5).fill(0).map((_, i) => <div key={i} className="h-16 bg-card border border-border rounded-sm animate-pulse" />)}
              </div>
            ) : (
              <div className="border border-border rounded-sm overflow-hidden">
                ...
                <div className="divide-y divide-border">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-right shrink-0">
                            <p className={`font-mono text-sm font-bold ${p.salePrice ? "text-primary" : ""}`}>
                              ${(p.salePrice ?? p.basePrice).toLocaleString()}
                            </p>
                            {p.salePrice && (
                              <p className="font-mono text-xs text-muted-foreground line-through">${p.basePrice.toLocaleString()}</p>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              ...
                              title="Edit price"
                              className="p-1.5 text-muted-foreground hover:text-primary border border-transparent hover:border-primary/30 rounded-sm transition-all"
                            >
                            ...
                            {confirmDelete === p.id ? (
                              <div className="flex items-center gap-1">
                                <button onClick={() => handleDelete(p.id)} className="p-1.5 text-destructive border border-destructive/30 rounded-sm hover:bg-destructive/10 transition-colors">
                                  <Check className="w-3.5 h-3.5" />
                                </button>
                                <button onClick={() => setConfirmDelete(null)} className="p-1.5 text-muted-foreground border border-border rounded-sm hover:bg-muted/30 transition-colors">
                              ...
                              <button
                                ...
                                className="p-1.5 text-muted-foreground hover:text-destructive border border-transparent hover:border-destructive/30 rounded-sm transition-all"
                              >
                  ...
                  {filtered.length === 0 && (
                    <div className="py-12 text-center font-mono text-sm text-muted-foreground">No products found</div>
                  )}
                </div>
              </div>
            )}
